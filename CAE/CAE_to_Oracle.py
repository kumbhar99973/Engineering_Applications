...
config_path = "caxrpt@10.0.130.36:/home/caxrpt/reportdata/Engineering_application_config_file/config_CAE.py"
destination = "/home/ec2-user/CAE/"

p_5 = subprocess.Popen(["sudo","scp","-i","/home/caxrpt/.ssh/id_rsa",config_path,destination])
sts_5 = os.waitpid(p_5.pid, 0)

sys.path.append("/home/ec2-user/CAE")
import config_CAE as config1
...
Logger = logger()
Logger.begin(process_name = 'CAE_TO_ORACLE')
...
###########################################################
    ...
    DELETE_AAM_CAE_USER_DETAILS_STG = """DELETE FROM AAM_CAD_CAE_ECS_USER_DETAILS_STG WHERE APP_TYPE = 'CAE'"""

    DELETE_AAM_CAE_LICENSES_DETAILS_STG = """DELETE FROM AAM_CAD_CAE_ECS_LICENSES_DETAILS_STG WHERE APP_TYPE = 'CAE'"""

    # Megging Simlab user staging table with final target table

    MERGE_AAM_CAE_USER_DETAILS = """
    ...
    destination = "/home/ec2-user/CAE/ORACLE/FILES/"
    log_path = "/home/ec2-user/CAE/ORACLE/FILES/"
    ...
    for i in all_sites:
        for j in config[i].keys():
            ...
            p_1 = subprocess.Popen(["sudo","scp","-i","/home/caxrpt/.ssh/id_rsa",file_user,destination])
            sts_1 = os.waitpid(p_1.pid, 0)

            p_2 = subprocess.Popen(["sudo","scp","-i","/home/caxrpt/.ssh/id_rsa",file_lic,destination])
            ...
            p_3 = subprocess.Popen(["sudo","scp","-i","/home/caxrpt/.ssh/id_rsa",file_user_rm,destination])
            sts_3 = os.waitpid(p_3.pid, 0)

            p_4 = subprocess.Popen(["sudo","scp","-i","/home/caxrpt/.ssh/id_rsa",file_lic_rm,destination])
            sts_4 = os.waitpid(p_4.pid, 0)

####################################################################################################
            Logger.start(log_info = 'Oracle Scripts')
####################################################################################################
            ...
            print('Database Conection is Done!!')
            ...
            try:
                cs_ORACLE.execute(DELETE_AAM_CAE_USER_DETAILS_STG)
                cs_ORACLE.execute(DELETE_AAM_CAE_LICENSES_DETAILS_STG)
...
####################################################################################################
                if os.path.isfile(f_full_usr) and os.path.getsize(f_full_usr) > 0:
                    ...
                    try:
                        ...
                        print(f'user file insert data shape is : {data.shape}')
                        if data.shape[0] > 0:
                            print(f'inserting users data of : {app_name}-{user_type} ')
                ...
                print ("Check 1.1")
                cs_ORACLE.execute(MERGE_AAM_CAE_USER_DETAILS.format(Curr_date_only))
                cs_ORACLE.execute(DELETE_AAM_CAE_USER_DETAILS_STG)
                ...
                if os.path.isfile(f_full_usr_rm) and os.path.getsize(f_full_usr_rm) > 0:
                    ...
                    try:
                        ...
                        print(f'user file remove data shape is : {data.shape}')
                        if data.shape[0]>0:
                            print(f'removing users data of : {app_name}-{user_type} ')
                            for index,row in data.iterrows():
                                user_name = row[0] ##date
                                try:
                        ...
                        pass
                cs_ORACLE.execute(MERGE_AAM_CAE_USER_DETAILS.format(prev_day_date))
                cs_ORACLE.execute(DELETE_AAM_CAE_USER_DETAILS_STG)
...
#################################################################################################### 
                ...
                if os.path.isfile(f_full_license) and os.path.getsize(f_full_license) > 0:
                    ...
                    try:
                        ...
                        print(f'license file insert data shape is : {data.shape}')
                        if data.shape[0]>0:
                            print(f'inserting license data of : {app_name}-{user_type} ')
                            for index,row in data.iterrows():
                                date = row[0] ##date
                                license_used = row[1]
                                license_total = row[2]
                ...
                cs_ORACLE.execute(MERGE_AAM_CAE_LICENSE_DETAILS.format(Curr_date_only))               
                cs_ORACLE.execute(DELETE_AAM_CAE_LICENSES_DETAILS_STG)
                ...
                if os.path.isfile(f_full_license_rm) and os.path.getsize(f_full_license_rm) > 0:
                    ...
                    try:
                        ...
                        print(f'license file insert data shape is : {data.shape}')
                        if data.shape[0] > 0:
                            print(f'removing license data of : {app_name}-{user_type} ')
                            for index,row in data.iterrows():
                                date = row[0] ##date
                                license_used = row[1]
                                license_total = row[2]
                ...
                cs_ORACLE.execute(MERGE_AAM_CAE_LICENSE_DETAILS.format(prev_day_date))               
                cs_ORACLE.execute(DELETE_AAM_CAE_LICENSES_DETAILS_STG)
...
####################################################################################################
            if  ((path.exists(f_full_usr_rm)) and (path.exists(f_full_usr))):
                transfer.upload_file('/home/ec2-user/CAE/ORACLE/FILES/'+f_usr_rm, 'aam-cae-files-oracle',f_usr_rm)
                os.remove('/home/ec2-user/CAE/ORACLE/FILES/'+f_usr_rm)   

            if  ((path.exists(f_full_license_rm)) and (path.exists(f_full_license))):
                transfer.upload_file('/home/ec2-user/CAE/ORACLE/FILES/'+f_lic_rm, 'aam-cae-files-oracle',f_lic_rm)
                os.remove('/home/ec2-user/CAE/ORACLE/FILES/'+f_lic_rm)
####################################################################################################
except Exception as e:
    Logger.update_error(error=e)  
finally:
