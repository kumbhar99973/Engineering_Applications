...
import config_PLM_old as config1
...
####################################################################################################
    ...
    for Day_of_the_Year_STR in range(213,234):
        ...
        for i in all_sites:
            for j in config[i].keys():
                ...
                f_usr_rm =  config[i][j]['user']+Curr_year_STR+"-"+Day_of_the_Year_STR+".txt"
                f_lic_rm =  config[i][j]['license']+Curr_year_STR+"-"+Day_of_the_Year_STR+".txt"
    ...
    #################################################################################################### 
                    ...
                    if os.path.isfile(f_full_license) and os.path.getsize(f_full_license) > 0:
                        ...
                        app_type   =  'PLM'
                        app_name   =  i
                        data = pd.read_csv(f_full_license, sep=' ')
                        if data.shape[0]>0:
                            for index,row in data.iterrows():
                                date = row[0] ##date
                                license_used = row[1]
                    ...
                    if os.path.isfile(f_full_license_rm) and os.path.getsize(f_full_license_rm) > 0:
                        ...
                        app_type   =  'PLM'
                        app_name   =  i
                        data = pd.read_csv(f_full_license_rm, sep=' ')
                        if data.shape[0] > 0:
